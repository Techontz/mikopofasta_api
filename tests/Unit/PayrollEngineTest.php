...
describe('payroll', function (): void {
    ...
    it('gives branch staff a transport allowance and HQ staff none', function (): void {
        $branch = payroll()->compute(staff('800000.00'), Money::zero(), true, false, false);
        $hq = payroll()->compute(staff('800000.00'), Money::zero(), false, false, false);
    ...
    it('recovers a loan and an advance at the flat rate, independently', function (): void {
        $both = payroll()->compute(staff('1000000.00'), Money::zero(), true, true, true);
        $loanOnly = payroll()->compute(staff('1000000.00'), Money::zero(), true, true, false);

        expect($both->deductionOf(DeductionType::Loan)->toDecimalString())->toBe('50000.00')
            ->and($both->deductionOf(DeductionType::Advance)->toDecimalString())->toBe('50000.00')
            ->and($both->deductionsTotal->toDecimalString())->toBe('200000.00')
    ...
    it('keeps its itemisation consistent with its totals', function (): void {
        $result = payroll()->compute(staff('1234567.89'), Money::of('999.99'), true, true, true);
    ...
    it('rounds the fund contribution half-up on a fractional salary', function (): void {
        ...
        $result = payroll()->compute(staff('833333.33'), Money::zero(), false, false, false);
    ...
    it('can produce a negative net when deductions exceed pay', function (): void {
        ...
        $result = payroll()->compute(staff('100000.00'), Money::zero(), false, true, true);

        expect($result->netSalary->toDecimalString())->toBe('10000.00');

        $worse = payroll()->compute(staff('50000.00'), Money::zero(), false, true, true);
    ...
    it('is deterministic — identical input gives identical output', function (): void {
        $once = payroll()->compute(staff('987654.32'), Money::of('1111.11'), true, true, false);
        $twice = payroll()->compute(staff('987654.32'), Money::of('1111.11'), true, true, false);
